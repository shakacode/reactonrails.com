...
> **For React-side migration patterns** (restructuring components, Context, data fetching, etc.), see the [RSC Migration Guide series](../../migrating/migrating-to-rsc.md). This page covers only the infrastructure upgrade.
...
| React version            | `npm ls react` / `yarn why react` / `pnpm list react` / `bun pm why react`                                                     | 19.0.4+ (see [v16.2.0 release notes](../../upgrading/release-notes/16.2.0.md) for security context) |
...
> **React 19.0.4+** is recommended. Earlier 19.0.x versions (19.0.0--19.0.3) have known security vulnerabilities — see the [v16.2.0 release notes](../../upgrading/release-notes/16.2.0.md) for details.
...
- **Custom webpack config incompatibility**: If your `serverWebpackConfig.js` was heavily customized, the generator's transforms may not apply cleanly. See [Preparing Your App: Step 4](../../migrating/rsc-preparing-app.md#step-4-set-up-the-rsc-webpack-bundle) for the underlying intent of each webpack change
...
If SSR hangs with large RSC payloads, you may need to update `react-on-rails-pro`. See [Stream Backpressure Deadlock](../../migrating/rsc-troubleshooting.md#stream-backpressure-deadlock) for details.

## What's Next

After the infrastructure is in place, migrate your React components:

1. **[Add `'use client'` to all entry points](../../migrating/rsc-preparing-app.md#step-5-add-use-client-to-all-registered-component-entry-points)** -- marks all existing components as Client Components so nothing changes yet
2. **[Switch to streaming rendering](../../migrating/rsc-preparing-app.md#step-6-switch-to-streaming-rendering)** -- update controllers and view helpers
3. **[Restructure components](../../migrating/rsc-component-patterns.md)** -- push `'use client'` boundaries down to leaf components
4. **[Migrate data fetching](../../migrating/rsc-data-fetching.md)** -- move from client-side fetching to server component patterns
