...
- On Heroku, ensure the renderer is started via `Procfile.web` (see [Heroku deployment](../building-features/node-renderer/heroku.md))
...
- Capture heap snapshots with `NODE_OPTIONS="--heapsnapshot-signal=SIGUSR2"` — send `kill -USR2 <worker-pid>` at different times and compare in Chrome DevTools (see [Debugging guide](../building-features/node-renderer/debugging.md#debugging-memory-leaks))
...
- If your component depends on URL or locale, include those in the `cache_key` (see [Caching docs](../building-features/caching.md))
